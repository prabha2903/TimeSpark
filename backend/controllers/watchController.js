...
export async function createWatch(req, res) {
    try {
      ...
      if(req.file?.filename) image = `${API_BASE}/uploads/${req.file.filename}`;
        ...
        console.error("Error in creating watch:", error);
...
export async function getWatches(req, res) {
  try {
    ...
    console.error("Error in fetching watches:", error);
    return res.status(500).json({
      success: false,
      message: "Server error. Please try again later.",
    });
...
//to delete a watch and also remove the linked image from uploads folder
export async function deleteWatch(req, res) {
    ...
    return res.status(200).json({
        ...
        console.error("Error in deleting watch:", error);
        return res.status(500).json({
            success: false,
            message: "Server error. Please try again later."
        });
...
export async function getWatchesByBrand(req, res) {
    try {
        ...
        console.error("Error in fetching watches by brand:", error);
        return res.status(500).json({
            success: false,
            message: "Server error. Please try again later."
        });
